...
Four analyses on the LLM-generated essays vs human essays. Default run is
fast (LIWC + TF-IDF + error dumps); SHAP is opt-in via `--shap` because it's slow.
...
4. **Style A error dumps** (default): for each trait, dump up to 10 misaligned
...
Run from `code/`:
    python -m src.analyze                                    # all fast analyses
