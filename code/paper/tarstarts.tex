...
\abstract{
Large language models are increasingly used to produce text on behalf of users or synthetic personas, yet it is unclear whether they can faithfully express personality expression in unconstrained writing.
We address this on the Pennebaker \& King stream-of-consciousness Essays dataset by training a multi-label RoBERTa probe to score trait expression in text, generating essays with GPT-4o-mini under single-trait (Style~B) and full-profile (Style~A) prompting, and comparing predicted trait distributions and per-essay alignment against human baselines.
